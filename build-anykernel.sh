...
###########################################################################################
# KrakenKernel Build Script (C) 2016   UpDated: 02/05/2016                                #
#  Modified By Bthorne79 Thanks NebulaKernel for the base  Original By RenderBroken                                    #
#                                                                                         #
...
function check_filesize() {
	...
	maxsize=18
	cd $ZIP_MOVE
	file="KrakenKernel_""$REV""_MR_""$VARIANT""_""$KVER"".zip"
...
function Build_Stand_Alone_Synapse() {
	...
	cp -vr $RAMDISK_Kraken_DIR $STAND_ALONE_UCI_DIR
	cd $STAND_ALONE_UCI_DIR
	echo "Creating Synapse Stand Alone Zip.."
	zip -r9 Kraken_Synapse_Scripts_Rev"$UCI_REV"_"$KVER".zip *
		mv Kraken_Synapse_Scripts_Rev"$UCI_REV"_"$KVER".zip $ZIP_MOVE
		rm -rf Kraken_Synapse_Scripts_Rev"$UCI_REV"_"$KVER".zip
		cd $KERNEL_DIR
TITLE="Kraken Synapse Stand Alone Created"
BACKTITLE="Kraken Synapse Stand Alone"
INFOBOX="Kraken_Synapse_Rev'$UCI_REV'_'$KVER'.zip \n\n Created Successfully"
message	
}
...
function menu_settings() {
		...
		if [ $TestBuild == '1' ]; then
			echo "This worked:" 
			#cp KrakenKernel_"$REV"_MR_"$VARIANT"_"$KVER".zip $COPY_ZIP
			else
			echo "Didnt Work" 
		fi
			exit		
...
dialog --title  "Build Not Completed"  --backtitle  "Build Had Errors" \
	...
	cd $ZIP_MOVE
	rm -rf KrakenKernel_"$REV"_MR_"$VARIANT"_"$KVER".zip
	cd $KERNEL_DIR
	fi
}
...
echo -e "${green}"
echo "KrakenKerrnel Creation Script:"
echo -e "${restore}"
...
echo -e "${green}"
echo "--------------------------------------------------------"
echo "KrakenKernel_'$REV'_MR_'$VARIANT'_'$KVER'-signed.zip"
echo "Created Successfully.."
